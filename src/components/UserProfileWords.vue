        ...
        <v-card v-if="currentWord" class="mt-2" dark>
            ...
            <v-card-actions>
                ...
                v-if="!currentWord.showTrans">
                    <v-icon>mdi-eye</v-icon>Показать перевод
                </v-btn>
                <v-btn small dark color="success" 
                @click="processWord(currentWord)">
                    <v-icon>mdi-check</v-icon>Я запомнил(а) это слово
                </v-btn>
            </v-card-actions>
        </v-card>
        <v-card>
            ...
            <v-card-text>
                <v-list>
                    <div v-for="{word, index} in words" :key="index">
                        <v-list-item-title @click="selectWord(word)">
    ...
    methods: {
        setWords() {
            ...
            for(var property in userWords) {
                if(Object.prototype.hasOwnProperty.call(userWords, property)) {
