...
export default {
    ...
    mutations: {
        ...
        UPDATE_USER_BOOK_PART_LAST_FINISH_INFO(state, payload) {
        ...
        ADD_USER_WORD(state, payload) {
            Vue.set(state.userData.words, payload.wordsId, payload.word)
        },
        REMOVE_USER_WORD(state, payload) {
            Vue.set(state.userData.words, payload)
    ...
    actions: {
        ...
        UPDATE_USER_BOOK_PART_STATS({commit, getters}, payload) {
            ...
            if (!getters.userData.books[payload.bookId].parts[payload.partId]) {
                ...
                .then(() => commit('ADD_USER_BOOK_PART', {bookId: payload.bookId, partId: payload.partId, timestamp: payload.timestamp}))
            }  
            userDataRef.update({
                [`books.${payload.bookId}.parts.${payload.partId}.lastOpenedDate`] : timestamp
            })
            .then(() => commit('UPDATE_USER_BOOK_PART_LAST_OPENED_DATE', {bookId: payload.bookId, partId: payload.partId, timestamp: payload.timestamp}))
        },
        FINISH_USER_BOOK_PART({commit, getters}, payload) {
            ...
            .then(() => commit('UPDATE_USER_BOOK_PART_LAST_FINISH_INFO', 
            {bookId: payload.bookId, partId: payload.partId, timestamp: payload.timestamp, rating: payload.rating}))
        },
